...
@Controller
@RequestMapping(value="/adm/room/")
public class RoomAdmController {
	...
	public String createRoom( Model model ) throws Exception {
		List<?> vo = roomAdmService.selectRoomNameList();
	...
	@ResponseBody
	public String insertRoom(String rm_rpp, String rm_array ,RoomAdmVO vo) throws Exception {
		...
		for(int i=0;i<json_arr_array.size();i++) {
			for(int j=0;j<json_arr_rpp.size();j++){
				...
				String result = roomAdmService.insertRoomList(vo); // 추후에 Map사용후 다중 insert 구현
	...
	public String detailRoom(String rm_name, RoomAdmVO vo, Model model ) throws Exception {
		System.out.println("rm_name : "+rm_name);		
		vo.setRm_name(rm_name); 
		
		List<?> rm_rpp = roomAdmService.selectRoomRPPList(vo); // 숙소에 대한 호실,인원수,가격 정보조회
		List<?> result = roomAdmService.selectRoomDetailList(vo);
		System.out.println("rm_rpp : "+rm_rpp);		
	...
	public String updateRoom(String rm_name, RoomAdmVO vo, Model model ) throws Exception {
		System.out.println("rm_name : "+rm_name);		
		vo.setRm_name(rm_name); 
	...
	@ResponseBody
	public String deleteRoom(String rm_name, RoomAdmVO vo, Model model ) throws Exception {
		String msg = "ok";
		vo.setRm_name(rm_name);
		int result = roomAdmService.deleteRoomNameList(vo);
